...
namespace std {
    template <typename T, typename Deleter, typename CharT>
    struct formatter<std_impl::unique_ptr<T, Deleter>, CharT> {
        ...
        auto format(const std_impl::unique_ptr<T, Deleter>& value, Context& ctx) const
            -> decltype(auto) {
