...
namespace std_impl::impl::shared_ptr {
    ...
    struct deleter_control_block final : control_block_base {
        T* ptr_{nullptr};
    ...
    export template <typename T, impl::unique_ptr::deleter Deleter>
    [[nodiscard]] auto make_deleter_control_block(T* ptr, const Deleter& deleter)
        -> control_block_base* {
        return new deleter_control_block<T, Deleter>{ptr, deleter};
    }

    export template <typename T, impl::unique_ptr::deleter Deleter>
    [[nodiscard]] auto make_deleter_control_block(T* ptr, Deleter&& deleter)
        -> control_block_base* {
        return new deleter_control_block<T, Deleter>{ptr, std::move(deleter)};
    }

    export template <typename T>
    [[nodiscard]] auto make_pointer_control_block(T* ptr) -> control_block_base* {
