...
namespace std_impl {
    ...
    public:
        ...
        template <impl::unique_ptr::deleter Deleter> shared_ptr(T* ptr, Deleter deleter);

        template <impl::unique_ptr::deleter Deleter, impl::shared_ptr::allocator Alloc> shared_ptr(T* ptr, Deleter deleter, const Alloc& alloc);
        ...
        template <typename Y> shared_ptr(const shared_ptr<Y>& other, element_type* ptr) noexcept;

        template <typename Y> shared_ptr(shared_ptr<Y>&& other, element_type* ptr) noexcept;
        ...
        [[nodiscard]] static auto adopt(T* ptr, impl::shared_ptr::control_block_base* control) noexcept -> shared_ptr {
    ...
    export template <typename T> class shared_ptr<T[]> {
    public:
        using element_type = T;
        ...
        template <impl::unique_ptr::deleter Deleter> shared_ptr(element_type* ptr, Deleter deleter);

        template <impl::unique_ptr::deleter Deleter, impl::shared_ptr::allocator Alloc> shared_ptr(element_type* ptr, Deleter deleter, const Alloc& alloc);

        shared_ptr(const shared_ptr& other) noexcept;
        shared_ptr(shared_ptr&& other) noexcept;

        template <typename Y, impl::unique_ptr::deleter Deleter> shared_ptr(unique_ptr<Y, Deleter>&& other);
    ...
    private:
        ...
        shared_ptr(element_type* ptr, impl::shared_ptr::control_block_base* control, bool add_ref) noexcept;

        element_type* ptr_{nullptr};
        impl::shared_ptr::control_block_base* control_{nullptr};
    };

    export template <typename T> auto swap(shared_ptr<T>& lhs, shared_ptr<T>& rhs) noexcept -> void;
    ...
    export template <typename T> [[nodiscard]] auto owner_hash(const shared_ptr<T>& ptr) noexcept -> std::size_t;

    export template <typename T> [[nodiscard]] auto owner_equal(const shared_ptr<T>& lhs, const shared_ptr<T>& rhs) noexcept -> bool;
}  // namespace std_impl
